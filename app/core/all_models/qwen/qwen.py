...
import queue
import threading
from transformers import TextIteratorStreamer,Qwen2VLForConditionalGeneration
import time
...
class QwenModelWrapper:
    ...
    def build_prompt(self, prompt, pil_image):
        messages = [
            {
                "role": "user",
                "content": [
    ...
    def generate_stream(self,model_obj, prompt, frame_queue=None, stop_event=None, max_new_tokens=256):
        ...
        def _generate():
            ...
            pil_imgs = pil_imgs[-1:]
            prepared_prompt = self.build_prompt(prompt, pil_imgs[0])

            image_inputs, video_inputs = process_vision_info(prepared_prompt)
    ...
    def load(self):
        ...
        self.tokenizer = Qwen2VLForConditionalGeneration.from_pretrained(self.model_path,use_fast=True)
        self.processor = AutoProcessor.from_pretrained(self.model_path,use_fast=True,min_pixels=self.min_pixels, max_pixels=self.max_pixels)
